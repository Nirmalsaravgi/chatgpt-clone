...
type UpsertParams = { userId: string; text: string; threadId?: string; metadata?: Record<string, any> }
...
export async function searchMemories(params: SearchParams): Promise<Array<{ id: string; text: string; score?: number; metadata?: any }>> {
  if (!MEM0_API_KEY) return []
  const { userId, query, threadId } = params
  try {
    const c = getClient()
    const filters: any = { OR: [{ user_id: userId }] }
    if (threadId) {
      // Scope by metadata only; 'tags' is not an allowed filter key
      filters.OR.push({ metadata: { threadId } })
    }
    const results = await c.search(query, { filters, api_version: "v2" as any })
